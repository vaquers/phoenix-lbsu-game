...
Игра поддерживает систему персонажей:

- базовый каталог: `public/assets/models/`
- имена файлов задаются в `src/game/characters/catalog.ts`
- активный персонаж выбирается в магазине

### Добавление нового персонажа

1. Добавь `.glb` в `public/assets/models/`.
2. Открой `src/game/characters/catalog.ts`.
3. Добавь новый объект в `CHARACTER_CATALOG`:
   - `id`, `name`, `price`
   - `assetPath` (путь к `.glb`)
   - `scale`, `rotationY`, `groundOffset`
4. Персонаж появится в магазине “Персонажи”.
...
4. Экспортируй в `public/assets/models/<name>.glb` и пропиши путь в каталоге.
